...
$settingsFilePath = "$env:USERPROFILE\GateBot\SRV\settings.json"
...
# Функция для чтения и записи файла settings.json
function Update-SettingsFile {
    param (
        [string]$userName,
        [string]$action # "add" для добавления, "remove" для удаления
    )
    # Чтение содержимого settings.json
    $settings = Get-Content $settingsFilePath -Raw | ConvertFrom-Json

    if ($action -eq "add") {
        # Проверка на наличие пользователя в списке
        if ($settings.valid_ids -notcontains $userName) {
            # Добавление пользователя в начало списка
            $settings.valid_ids = @($userName) + $settings.valid_ids
            Write-Host "`n" (Center-Text "Пользователь $userName добавлен в settings.json.") "`n"
        }
    } elseif ($action -eq "remove") {
        if ($settings.valid_ids -contains $userName) {
            # Удаление пользователя из списка
            $settings.valid_ids = $settings.valid_ids | Where-Object { $_ -ne $userName }
            Write-Host "`n" (Center-Text "Пользователь $userName удален из settings.json.") "`n"
        } else {
            Write-Host "`n" (Center-Text "Пользователь $userName не найден в settings.json.") "`n"
        }
    }

    # Сохранение изменений
    $settings | ConvertTo-Json -Depth 100 | Set-Content -Path $settingsFilePath -Encoding UTF8
}
...
function Create-User {
    ...
    if (Get-LocalUser -Name $userName -ErrorAction SilentlyContinue) {
        ...
        # 1.5 Обновление файла settings.json
...
function Remove-User {
    ...
    if (Get-LocalUser -Name $userName -ErrorAction SilentlyContinue) {
        ...
        # Удаление пользователя из settings.json
